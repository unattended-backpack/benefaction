...
contract Poseidon2 is
  ...
  */
  function hash (
    ...
    return _poseidon2Core(_x, 0, 0, 1 << 64);
  }
  ...
  */
  function hash (
    ...
    return _poseidon2Core(_x, _y, 0, 2 << 64);
  }
  ...
  */
  function hash (
    ...
    return _poseidon2Core(_x, _y, _z, 3 << 64);
  }

  /**
    Compute the Poseidon2 permutation over BN254. The four parameters form the
    initial sponge state: the input field elements, zero-padding, and the
    initialization vector `numInputs << 64` in the capacity element.

    @param _s0 The first sponge state element.
    @param _s1 The second sponge state element.
    @param _s2 The third sponge state element.
    @param _s3 The capacity element (initialization vector).

    @return _ The first element of the final sponge state (the hash output).
  */
  function _poseidon2Core (
    ...
    uint256 _resultOutput;
    assembly {
      let _PRIME :=
        0x30644e72e131a029b85045b68181585d2833e84879b9709143e1f593f0000001
      let _state0 := mod(_s0, _PRIME)
      let _state1 := mod(_s1, _PRIME)
      let _state2 := mod(_s2, _PRIME)
      let _state3 := mod(_s3, _PRIME)
...

      {
...

        {
          ...
          _t3 := addmod(_t3, _t0, _PRIME)
          let _t4 := mulmod(_t1, 4, _PRIME)
          _t4 := addmod(_t4, _t3, _PRIME)
          let _t5 := mulmod(_t0, 4, _PRIME)
          _t5 := addmod(_t5, _t2, _PRIME)
          let _t6 := addmod(_t3, _t5, _PRIME)
          let _t7 := addmod(_t2, _t4, _PRIME)
          _state0 := _t6
          _state1 := _t5
      ...
      // Internal rounds
      {
        ...
        {
          let _sum := add(_state0, _state1)
          _sum := add(_sum, _state2)
          _sum := addmod(_sum, _state3, _PRIME)
          _state0 := addmod(
            mulmod(
              _state0,
